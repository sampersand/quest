...
impl Debug for Primitive {
	fn fmt(&self, f: &mut Formatter) -> fmt::Result {
		match self {
			Primitive::Text(t) => Debug::fmt(&t, f),
			Primitive::Number(n) => Debug::fmt(&n, f),
			Primitive::Variable(v) => Debug::fmt(&v, f),
			Primitive::Regex(r) => Debug::fmt(&r, f),
			Primitive::StackPos(s) => Debug::fmt(&s, f),
		}
	}
}
