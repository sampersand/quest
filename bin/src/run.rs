...
pub fn file<P: AsRef<Path>>(path: P, args: Args) -> Result<Object> { 
	run(BufStream::try_from(path.as_ref())?, args).map_err(From::from)
}

pub fn expression(expr: String, args: Args) -> Result<Object> {
	run(BufStream::from(expr), args).map_err(From::from)
}

pub fn stdin(args: Args) -> Result<Object> {
	run(BufStream::stdin(), args).map_err(From::from)
}

pub fn repl(args: Args) -> Result<Object> {
	run(Repl::new(), args).map_err(From::from)
}

fn run<R: Runner>(runner: R, args: Args) -> quest_core::Result<Object> {
