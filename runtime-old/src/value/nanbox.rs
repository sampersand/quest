...
/// The type that represents a value in Quest bytecode.
///
/// Quest behaves as if every type is heap-allocated, including `Number`s, `Boolean`s, `Null` and `RustFn`s. Because
/// those types are so rarely modified directly, it's much more efficient store them on the stack, and only heap
/// allocate them when their instances have attributes added directly to them.
///
/// To allow for multiple different internal implementations of `Value` (such as when the Nanboxing technique won't work
/// ), implementations must adhere to the `ValueImplementation` trait.
// NB: The PhantomData's used to make us `!Sync` and `!Send`.
pub struct Value(ValueInner, PhantomData<*mut ()>);

sa::assert_eq_align!(Value, u64, f64, *const ());
sa::assert_eq_size!(Value, u64, f64, *const ());
...
impl Tag {
	...
	// Any tag other than these is considered a normal float.
	...
	const _UNUSED1: Self = Self(Self::CLONE_MASK | 0b111);
	...
	const _UNUSED2: Self = Self(Self::COPY_MASK  | 0b101);
	const _UNUSED3: Self = Self(Self::COPY_MASK  | 0b110);
	const _UNUSED4: Self = Self(Self::COPY_MASK  | 0b111);
	...
	#[inline]
	const fn is_normal_float(&self) -> bool {
...
impl Debug for Value {
	fn fmt(&self, f: &mut Formatter) -> fmt::Result {
		match self.tag() {
			...
			_ => write!(f, "Float({})", self.as_float().unwrap())
		}
	}
}
...
mod tests {
	...
	#[test]
	fn all_possible_tags_are_correct() {
		macro_rules! asserts {
		...
		for tag in 0..u16::MAX {
			let tag = Tag(tag);
			match tag {
				Tag::OBJECT   => asserts!(!tag.is_normal_float(),  tag.is_clone(),  tag.is_data_pointer()),
				Tag::TEXT     => asserts!(!tag.is_normal_float(),  tag.is_clone(),  tag.is_data_pointer()),
				Tag::REGEX    => asserts!(!tag.is_normal_float(),  tag.is_clone(),  tag.is_data_pointer()),
				Tag::BLOCK    => asserts!(!tag.is_normal_float(),  tag.is_clone(),  tag.is_data_pointer()),
				Tag::LIST     => asserts!(!tag.is_normal_float(),  tag.is_clone(),  tag.is_data_pointer()),
				Tag::MAP      => asserts!(!tag.is_normal_float(),  tag.is_clone(),  tag.is_data_pointer()),
				Tag::SCOPE    => asserts!(!tag.is_normal_float(),  tag.is_clone(),  tag.is_data_pointer()),
				Tag::_UNUSED1 => asserts!(!tag.is_normal_float(),  tag.is_clone(),  tag.is_data_pointer()),
				Tag::CONSTS   => asserts!(!tag.is_normal_float(), !tag.is_clone(), !tag.is_data_pointer()),
				Tag::INT      => asserts!(!tag.is_normal_float(), !tag.is_clone(), !tag.is_data_pointer()),
				Tag::RUSTFN   => asserts!(!tag.is_normal_float(), !tag.is_clone(),  tag.is_data_pointer()),
				Tag::ZST      => asserts!(!tag.is_normal_float(), !tag.is_clone(), !tag.is_data_pointer()),
				Tag::LITERAL  => asserts!(!tag.is_normal_float(), !tag.is_clone(), !tag.is_data_pointer()),
				Tag::_UNUSED2 => asserts!(!tag.is_normal_float(), !tag.is_clone(), !tag.is_data_pointer()),
				Tag::_UNUSED3 => asserts!(!tag.is_normal_float(), !tag.is_clone(), !tag.is_data_pointer()),
				Tag::_UNUSED4 => asserts!(!tag.is_normal_float(), !tag.is_clone(), !tag.is_data_pointer()),
				_             => asserts!( tag.is_normal_float(), !tag.is_clone(), !tag.is_data_pointer())
			}
		}
	}
