...
impl Number {
	...
	pub fn from_str_radix(inp: &str, radix: u32) -> Result<Self, FromStrError> {
		if radix < 2 || radix > 36 {
			return Err(FromStrError::Radix(radix))
		}

		IntegerType::from_str_radix(inp.trim(), radix)
			.map(Self::from)
			.map_err(FromStrError::Integer)
	}
...
impl Display for FromStrError {
	fn fmt(&self, f: &mut Formatter) -> fmt::Result {
		match self {
			FromStrError::Integer(err) => Display::fmt(err, f),
			FromStrError::Float(err) => Display::fmt(err, f),
			FromStrError::Radix(radix) => write!(f, "bad radix: {}", radix)
		}
	}
}
...
impl std::error::Error for FromStrError {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
		match self {
			FromStrError::Integer(ref err) => Some(err),
			FromStrError::Float(ref err) => Some(err),
			FromStrError::Radix(_) => None
		}
	}
}
...
mod tests {
	...
	fn from_str_radix() {
		...
		// invalid bases
		assert_eq!(Number::from_str_radix("0", 0).unwrap_err(), FromStrError::Radix(0));
		assert_eq!(Number::from_str_radix("0", 1).unwrap_err(), FromStrError::Radix(1));
		assert_eq!(Number::from_str_radix("0", 37).unwrap_err(), FromStrError::Radix(37));
	}
	...
	fn try_from() {
		...
		// bad numbers
		assert!(matches!(Number::try_from("invalid").unwrap_err(), FromStrError::Float(..)));
		assert!(matches!(Number::try_from("1.2.3").unwrap_err(), FromStrError::Float(..)));
		assert!(matches!(Number::try_from("12e3e4").unwrap_err(), FromStrError::Float(..)));
		assert!(matches!(Number::try_from("").unwrap_err(), FromStrError::Float(..)));
		assert!(matches!(Number::try_from(" ").unwrap_err(), FromStrError::Float(..)));
	}
